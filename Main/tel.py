...
def start_survey_handler(update, context):
    """Send a message when the command /start is issued."""
    add_typing(update, context)
    buttons = MultiItems("What is the main reason you are here", ["Reduce stress", "Improved mood", "Sleep better", "Relationships","Next Question"])
...
def start_command_handler(update, context):
    """Send a message when the command /start is issued."""
    add_typing(update, context)
    buttons = MultiItems("What is the main reason you are here", ["Reduce stress", "Improved mood", "Sleep better", "Relationships","Next","End Survey"])
    buttons_a = MultiItems("How have you been feeling lately", ["Angry", "Content", "Sad", "Hopeless","Worried","Next","End Survey"])
    buttons_a = MultiItems("Are you dealing with any recent life events", ["Angry", "Content", "Sad", "Hopeless","Worried","Next","End Survey"])
    add_suggested_actions(update, context, buttons)
...
class DefaultConfig:
    ...
    TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "1981892694:AAHJneQhQ2ZbNi-IjqIZ9qPQ0a3G8ArdCeA")
